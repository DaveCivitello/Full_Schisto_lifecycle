...
Worm_development = function(Human.stats, parameters){
  ...
  for(i in 1:dim(Human.stats)[1]){
    ...
    juv.develop = rbinom(n = length(juveniles), size = juveniles, prob = parameters["worm_r"])
...
run_DEB_IBM = function(N.snail=60, N.comp=0, N.human=10, min.L=2, max.L=16, Food=1, drip=T, pars){
  ...
  for(tick in 1:pars["timespan"]){
    ...
    state$Humans[[tick+1]] = Worm_development(state$Humans[[tick]], pars)
